    ...
    public class EvolutionManager : Singleton<EvolutionManager>
    {
        ...
        private void EvolvePopulation()
        {
            ...
            while (newPop.Count < population.Count)
            {
                ...
                int newBuildTicks = child.ticksBetweenBuilds + Random.Range(-1, 2);
                child.ticksBetweenBuilds = (int)Mathf.Clamp(newBuildTicks, 1, 20);
                int newQueenBuildTicks = child.ticksBetweenQueenBuilds + Random.Range(-2, 3);
                child.ticksBetweenQueenBuilds = (int)Mathf.Clamp(newQueenBuildTicks, 1, 50);
                newPop.Add(child);
            }
